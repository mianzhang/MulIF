...
Per uid group, **GII**, **VIA**, and **ICR** (instruction coverage rate: share of instructions
passed in at least one rollout) are computed and logged as ``gii`` / ``via`` / ``icr`` per sample.
"""
...
class IfverifySgRankRewardManager(AbstractRewardManager):
    ...
    used in the tensor. Also logs **GII**, **VIA**, and **ICR** per group.
