        ...
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if isinstance(var_vals[0], str):
                    continue
                # Skip non-scalar values (e.g. follow_instruction_list: list of bools per sample)
                # so we don't run np.mean/bootstrap on them or log nonsensical metrics to wandb.
                if isinstance(var_vals[0], (list, dict)):
                    continue
...
def compute_source_metrics(batch: DataProto) -> dict[str, Any]:
    ...
    if 'token_level_scores' in batch.batch:
        ...
        for reward, source in zip(sample_rewards, sources, strict=True):
    ...
    for key in (
        ...
        source_vals = defaultdict(list)
        for val, source in zip(values, sources, strict=True):
            source_vals[source].append(float(val))
        for source, vals in source_vals.items():
            metrics[f'source/{source}/{key}'] = np.mean(vals)
