...
torchrun --standalone --nnodes=1 --nproc-per-node=1 -m verl.trainer.sft_trainer \
    data.train_files="verl_data/rebuttal_random_sft.parquet" \
    data.val_files=null \
    ...
    trainer.experiment_name="rebuttal_random_sft" \
    trainer.total_epochs=1 \
    trainer.default_local_dir="checkpoints/rebuttal_random_sft" \
