...
if [ -n "$AZURE_STORAGE_ROOT" ]; then
    ...
    DEFAULT_RUN_LOG_DIR="$AZURE_STORAGE_ROOT/logs/rebuttal_SaR_qwen17b_random_sft"
    DEFAULT_WANDB_DIR="$AZURE_STORAGE_ROOT/wandb"
    DEFAULT_CKPT_DIR="$AZURE_STORAGE_ROOT/checkpoints/rebuttal_SaR_qwen17b_random_sft"
    DEFAULT_PROFILE_DIR="$AZURE_STORAGE_ROOT/profile/rebuttal_SaR_qwen17b_random_sft"
else
    ...
    DEFAULT_CKPT_DIR="$ROOT_DIR/checkpoints/rebuttal_SaR_qwen17b_random_sft"
    DEFAULT_PROFILE_DIR="$ROOT_DIR/outputs/profile/rebuttal_SaR_qwen17b_random_sft"
fi
...
n_gpus_per_node=4
MODEL_PATH=$HF_CACHE_DIR/billmianz/rebuttal_random_sft
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='rebuttal_SaR_qwen17b_random_sft' \
    ...
    trainer.total_training_steps=400 > "$RUN_LOG_DIR/rebuttal_SaR_qwen17b_random_sft.log"
