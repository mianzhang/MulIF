...
if [ -n "$AZURE_STORAGE_ROOT" ]; then
    ...
    DEFAULT_RUN_LOG_DIR="$AZURE_STORAGE_ROOT/logs/IFTrain_log_base_qwen17b_sft1"
    DEFAULT_WANDB_DIR="$AZURE_STORAGE_ROOT/wandb"
    DEFAULT_CKPT_DIR="$AZURE_STORAGE_ROOT/checkpoints/IFTrain_log_base_qwen17b_sft1"
    DEFAULT_PROFILE_DIR="$AZURE_STORAGE_ROOT/profile/IFTrain_log_base_qwen17b_sft1"
else
    ...
    DEFAULT_CKPT_DIR="$ROOT_DIR/checkpoints/IFTrain_log_base_qwen17b_sft1"
    DEFAULT_PROFILE_DIR="$ROOT_DIR/outputs/profile/IFTrain_log_base_qwen17b_sft1"
fi
...
python3 -m verl.trainer.main_ppo \
    ...
    +reward_model.reward_kwargs.inst_weight_mode=log \
    trainer.critic_warmup=0.0 \
    trainer.logger=['console','wandb'] \
    trainer.project_name='MulIF' \
    trainer.experiment_name='IFTrain_log_base_qwen17b_sft1' \
    ...
    trainer.total_training_steps=500 > "$RUN_LOG_DIR/IFTrain_log_base_qwen17b_sft1.log"
