...
if [ -n "$AZURE_STORAGE_ROOT" ]; then
    ...
    DEFAULT_RUN_LOG_DIR="$AZURE_STORAGE_ROOT/logs/bg_qwen17b"
    DEFAULT_WANDB_DIR="$AZURE_STORAGE_ROOT/wandb"
    DEFAULT_CKPT_DIR="$AZURE_STORAGE_ROOT/checkpoints/bg_qwen17b"
    DEFAULT_PROFILE_DIR="$AZURE_STORAGE_ROOT/profile/bg_qwen17b"
else
    ...
    DEFAULT_CKPT_DIR="$ROOT_DIR/checkpoints/MulIF/bg_qwen17b"
    DEFAULT_PROFILE_DIR="$ROOT_DIR/outputs/profile/bg_qwen17b"
fi
...
python3 -m verl.trainer.main_ppo \
    ...
    +reward_model.reward_kwargs.use_hit_rewards=True \
    trainer.critic_warmup=0.0 \
    trainer.logger=['console','wandb'] \
    trainer.project_name='MulIF' \
    trainer.experiment_name='bg_qwen17b' \
    ...
    trainer.total_training_steps=125 > "$RUN_LOG_DIR/bg_qwen17b.log"
